...
function registerApi(name, endpoint, method, description, parameters, responseExample, keywords, headers // 헤더 정보 파라미터 추가
) {
    // API 정보 등록
    apiDatabase.push({
        name,
...
// API 등록
// 한국투자증권 접근토큰발급 API
registerApi("korea_investment_oauth_token", "/oauth2/tokenP", "POST", "한국투자증권 접근토큰발급 API - API 호출에 필요한 OAuth 접근 토큰을 발급합니다.", {
    "grant_type": "client_credentials (고정값)",
    "appkey": "발급받은 appkey",
    "appsecret": "발급받은 appsecret"
}, '{"access_token":"발급된 접근 토큰","token_type":"Bearer","expires_in":86400,"access_token_token_expired":"2023-12-31 23:59:59"}', ["토큰", "접근토큰", "인증", "OAuth", "로그인", "API키", "API 키", "인증키", "한국투자증권"], {
...
// 한국투자증권 주식잔고조회 API
registerApi("korea_investment_stock_balance", "/uapi/domestic-stock/v1/trading/inquire-balance", "GET", "한국투자증권 주식잔고조회 API - 계좌의 보유 주식 종목 및 잔고 정보를 조회합니다.", {
    "CANO": "계좌번호(8자리)",
    "ACNT_PRDT_CD": "계좌상품코드(2자리)",
...
}, '{"output1":[{"pdno":"종목번호","prdt_name":"종목명","hldg_qty":"보유수량","pchs_avg_pric":"매입평균가격","pchs_amt":"매입금액","prpr":"현재가","evlu_pfls_amt":"평가손익금액","evlu_pfls_rt":"평가손익율"}],"output2":{"dnca_tot_amt":"예수금총금액","scts_evlu_amt":"유가평가금액","tot_evlu_amt":"총평가금액","nass_amt":"순자산금액"}}', ["주식", "잔고", "잔고조회", "계좌", "보유종목", "포트폴리오", "주식잔고", "자산", "한국투자증권"], {
    "authorization": "Bearer {접근토큰}",
    "appkey": "발급받은 appkey",
    "appsecret": "발급받은 appsecret",
...
// 한국투자증권 매수가능조회 API
registerApi("korea_investment_inquire_psbl_order", "/uapi/domestic-stock/v1/trading/inquire-psbl-order", "GET", "한국투자증권 매수가능조회 API - 주식 매수 가능 수량 및 금액을 조회합니다.", {
    "CANO": "계좌번호(8자리)",
    "ACNT_PRDT_CD": "계좌상품코드(2자리)",
...
}, '{"output":{"ord_psbl_cash":"주문가능현금(예수금)","nrcvb_buy_amt":"미수없는매수금액(미수 사용X)","nrcvb_buy_qty":"미수없는매수수량(미수 사용X)","max_buy_amt":"최대매수금액(미수 사용O)","max_buy_qty":"최대매수수량(미수 사용O)"}}', ["주식", "매수", "매수가능", "주문가능", "가능금액", "가능수량", "예수금", "매수수량", "한국투자증권"], {
    "authorization": "Bearer {접근토큰}",
    "appkey": "발급받은 appkey",
    "appsecret": "발급받은 appsecret",
...
// 한국투자증권 주식주문(신용) API
registerApi("korea_investment_stock_order_credit", "/uapi/domestic-stock/v1/trading/order-credit", "POST", "한국투자증권 주식주문(신용) API - 신용거래를 통한 주식 매수/매도 주문을 실행합니다. (모의투자 미지원)", {
    "CANO": "계좌번호(8자리)",
    "ACNT_PRDT_CD": "계좌상품코드(2자리)",
    "PDNO": "종목코드(6자리)",
...
}, '{"output":{"krx_fwdg_ord_orgno":"거래소코드","odno":"주문번호","ord_tmd":"주문시간"}}', ["주식", "주문", "신용", "신용주문", "융자", "대주", "신용거래", "신용매수", "신용매도", "한국투자증권"], {
    "content-type": "application/json; charset=utf-8",
    "authorization": "Bearer {접근토큰}",
    "appkey": "발급받은 appkey",
    "appsecret": "발급받은 appsecret",
