...
// 서버에서 사용할 도구 목록
const TOOLS = [SEARCH_API_TOOL, LIST_ALL_APIS_TOOL];
// API 검색 핸들러
...
async function runServer() {
    try {
        ...
        const apiData = JSON.parse(apisJson);
        apiData.forEach(api => {
            const { keywords, ...apiInfo } = api;
            registerApi(apiInfo.name, apiInfo.endpoint, apiInfo.method, apiInfo.description, apiInfo.parameters, apiInfo.responseExample, keywords, apiInfo.headers);
        });
